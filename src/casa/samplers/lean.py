...
class LeanARS:
    ...
    #       it becomes tricky to handle. We could block a whitespace and its
    #       non-whitespace suffixes, but the sampler just samples a different
    #       whitespace character (e.g., tab) and repeats the same mistake.
    ...
    def _block_token_at_node(
        self,
        node: TrieNode,
        token_to_block: int,
        tokens_to_node: list[int],
        prefix_str: str,
        depth: int,
    ) -> PrefixLearnEvent:
        """Block a token at a node, propagate up, and record stats using root mass diff."""
        # Measure mass before blocking
        raw_prob_of_blocked = (
            torch.exp(node.raw_logprob[0, token_to_block]).item()
        ...
        total_blocked_before = (
            torch.exp(node.raw_logprob[0][blocked_mask_before]).sum().item()
            if node.raw_logprob is not None
            else 0.0
        )
        ...
        self._log(
            f"[update] Blocked token {token_to_block} = "
            f"{repr(self.llm.tokenizer.decode([token_to_block]))}"
        )

        self._propagate_up(node, tokens_to_node)
    ...
    def update_from_generated_tokens(
    ...
    ) -> Optional[PrefixLearnEvent]:
        """
        When an entire attempted proof is incorrect, block the entire sequence.
        ...
        prefix_str = self.llm.tokenizer.decode(generated_tokens)

        return self._block_token_at_node(
            node=node,
            token_to_block=token_to_block,
            tokens_to_node=generated_tokens[:-1],
            prefix_str=prefix_str,
            depth=len(generated_tokens) - 1,
        )
    ...
    def update_from_invalid_prefix(
        ...
        return self._block_token_at_node(
            node=node,
            token_to_block=token_to_block,
            tokens_to_node=generated_tokens[:split_idx],
            prefix_str=invalid_prefix,
            depth=split_idx,
        )
    ...
    # TODO: The Lean proof checker functions as our oracle. Any prefix and its
    #       continuations we block are dependent on the shortest invalid prefix
    #       it returns. While checking incrementally at token level, sometimes an
    #       incomplete tactic produces an invalid prefix that should not be blocked
    #       ideally (it should return None instead). We need to refine the checker
    #       to avoid such cases. For now, we check at tactic boundaries (newline or
    #       semicolon) to reduce such cases.
    #
    #       We also need to profile the sampler and checker in loop to see which
    #       part acts as the bottleneck.
    ...
    def generate_and_check(
        ...
        with torch.no_grad():
            for step in range(self.max_new_tokens):
                if step % 20 == 0:
                    self._log(
                        f"[generate] Step {step}/{self.max_new_tokens}: "
                        f"{repr(self.llm.tokenizer.decode(generated_tokens))}..."
                    )
                ...
                if next_token == self.llm.tokenizer.eos_token_id:
                    ...
                    if not result.is_valid and result.shortest_invalid_prefix is not None:
                        return GenerateAndCheckResult(
                            valid_proof=None,
    ...
    def sample(
        ...
        for sample_idx in range(n_samples):
            ...
            for attempt in range(max_attempts):
                ...
                self._log(
                    f"[sample] Generated:\n"
                    f"{self.llm.tokenizer.decode(result.generated_tokens)}"
                )
                ...
                if result.invalid_prefix:
                    ...
                    if self.learn:
                        self._log(
                            f"[sample] Learning prefix: "
                            f"{repr(result.invalid_prefix[:30])}"
                        )
                        self.update_from_invalid_prefix(
                            result.invalid_prefix, result.generated_tokens
                        )
                else:
                    # No invalid prefix returned — try blocking the full sequence
                    if self.learn:
                        event = self.update_from_generated_tokens(
                            result.generated_tokens
                        )
                        if event:
                            invalid_prefixes_found.append(event.invalid_prefix)
                            self._log(
                                f"[sample] Blocked full sequence at depth "
                                f"{event.block_depth}"
                            )
